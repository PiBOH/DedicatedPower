...
### Added
- Added MOTD editor buttons for all Minecraft legacy colors (`§0`-`§f`)
- Added the missing `Obfuscated` (`§k`) formatting control; Bold, Underline, Italic, Strikethrough, and Reset are now all available
- Added automated coverage for the complete color and formatting MOTD generated by MCTools
